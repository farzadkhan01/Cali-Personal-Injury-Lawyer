...
export const metadata = {
  title: "Insights",
  description: "Educational articles on personal injury topics, from car accidents to the general case process.",
        ...
        <Container size="narrow">
          ...
          <Reveal delay={0.2}>
            <p className="mt-6 max-w-2xl text-lg leading-relaxed text-gray-600">
              General, educational articles on personal injury topics. Nothing here is a substitute for a
              ...
              <Link href={`/insights/${featured.slug}`} className="group block h-full">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-warm-white/50">{featured.readTime}</span>
                  </div>
                  <div className="flex flex-col gap-3">
                    <span className="font-display text-3xl text-warm-white">{featured.title}</span>
                    <p className="text-base leading-relaxed text-warm-white/70">{featured.excerpt}</p>
                  </div>
            ...
            {rest.map((article, index) => (
              <Reveal key={article.slug} delay={index * 0.05}>
                <Link href={`/insights/${article.slug}`} className="group block h-full">
                  <GlassCard tone="light" blur="soft" hover className="flex h-full flex-col justify-between gap-6 p-6">
                    <div className="flex items-center gap-3">
                      <Badge>{article.category}</Badge>
                      <span className="text-xs text-gray-400">{article.readTime}</span>
                    </div>
                    <div className="flex flex-col gap-2">
                      <span className="font-display text-xl text-ink-950">{article.title}</span>
