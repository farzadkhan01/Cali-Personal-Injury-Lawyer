      ...
      <Container>
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-3">
          {testimonials.map((item, index) => (
            <Reveal key={item.name + index} delay={index * 0.1}>
              <GlassCard tone="light" blur="soft" hover className="flex h-full flex-col gap-6 p-8">
                <div className="flex items-center justify-between">
                  <Quote className="h-6 w-6 text-accent-500" aria-hidden="true" />
                  <Badge className="px-3 py-1 text-[0.65rem]">Illustrative</Badge>
                </div>
                <p className="font-display text-lg leading-relaxed text-ink-950">{item.quote}</p>
